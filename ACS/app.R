...
ui=tagList(tags$style(HTML("body{font-family:Bahnschrift;font-style: normal;}")),
  navbarPage(header=tags$head(tags$style(type="text/css", "body {padding-top: 70px;}",".tab-content { overflow: visible; }"),
             ...
             title=tags$div(style="display: inline-block;vertical-align:top;","Associative Classification Software"),
             ...
             tabPanel(div("Home"),value = "introtab",column(width = 12,
             ...
             tabPanel(div("Data"),value = "datatab",
                      fluidRow(br(),
                        column(2, shinyLP::panel_div("success", div(style="font-family: Bahnschrift;",strong("File upload")),content = 
                        ...
                        column(6, shinyLP::panel_div(class_type = "info", div(style="font-family: Bahnschrift;",strong("Data view")),
                                                     content = tagList(column(width = 12,DT::dataTableOutput("data_view"))
                                                     ,br(),br(),br(),br(),br(),br(),br(),br(),br(),br(),br(),br(),br(),br(),br(),br(),br(),br(),br(),br(),br(),br(),br(),br(),br(),br(),br(),br(),br(),br(),br(),br()
                                                     )
                        )),
                        column(4,
                               shinyLP::panel_div(class_type = "warning", div(style="font-family: Bahnschrift;",strong("Variable types")),
             ...
             tabPanel(div("Analysis"),value = "analysistab",br(),
                      sidebarLayout(
                        sidebarPanel(tags$style(".well {background-color:#E0FFFF;}"),width = 3,
                                     ...
                                     tags$div(style="display:inline-block",numericInput(inputId = "seed",label = "Random seed:",value = sample(.Machine$integer.max,1))),
...
server=function(input, output, session){
  ...
  output$used_libraries=DT::renderDataTable(server = F,{
    mat=matrix(NA,nrow = 3,ncol = 2,dimnames = list(NULL,c("Section","Package")))
    mat[,1]=c("Graphical interface", "Data upload, Data tables","Feature selection, Association rules, Discretization techniques, 
              Associative classification, Network visualization")
    mat[,2]=c("shiny, shinythemes, shinyWidgets, shinyLP, shinyalert, shinyjs, shinysky,","foreign, readxl, DT",
  ...
  rownames=FALSE, selection="none", server = FALSE, escape = FALSE,
...
#  })
  ...
  observeEvent(input$analysis,{
    ...
    if(any(c(input$gr_variable=="",is.null(input$other_variables),is.na(input$support),is.na(input$confidence)))==TRUE){
      ...
      if(isolate(input$fselection)==TRUE && isolate(input$fs_options)=="boruta"){
      ...
      }else if(isolate(input$fselection)==TRUE && isolate(input$fs_options)=="lasso"){
        if(length(lev())==2){
          ...
          data=data[,c(cls,rownames(coefs)[which(coefs[,1]!=0)])]
          after_FS$predictors=rownames(coefs)[which(coefs[,1]!=0)]
        }else if(length(lev())>2){
          cv.lasso=glmnet::cv.glmnet(X, y, alpha = 1, family = "multinomial")
          model=glmnet::glmnet(X, y, alpha = 1, family = "multinomial",lambda = cv.lasso$lambda.min)
          coefs=sapply(model$beta, as.vector)
          remove=apply(weights, MARGIN = 1, FUN = function(x) all(x == 0))
          data=cbind.data.frame(cls=data[cls],X[!remove,])
          after_FS$predictors=colnames(data)[-1]
        }
      }else{
        data=data
      }
      ...
      if(inherits(model(),c("try-error", "error"))==FALSE){
        ...
        output$selected_features=renderUI({
          if(isolate(input$fselection)==FALSE || length(after_FS$predictors)==0){
            return(NULL)
          }else if(isolate(input$fselection)==TRUE){
            if(length(after_FS$predictors)==length(input$other_variables)){
              wellPanel(
                tags$h5(strong("Feature selection output"),align="justify"),br(),
                tags$h5("All predictive variables were selected.",align="justify")
              )
            }else{
              wellPanel(
                ...
                tags$h5(tags$div(style="font-family: Bahnschrift;",strong("Selected features: "),
                                 HTML(paste(after_FS$predictors,collapse = ", ")),align="justify"))
              )
            }
          }
        })
    ...
    lapply(1:3, function(i){
      ...
      output[[paste0("dwnl_bttn",i)]]=downloadHandler(
        filename = function() {
          paste(paste0("report_acs_", format.Date(Sys.Date(),"%m/%d/%Y")), sep = ".",ext[i]
          )
        },
  ...
  parameters=reactive({
    ...
    mat=matrix(NA,6,2)
    mat[,1]=c("Response/Output variable","Positive class","Support","Confidence","Feature selection","Classification algorithm")
    ...
    mat[5,2]=ifelse(input$fselection==TRUE,"Yes","No")
